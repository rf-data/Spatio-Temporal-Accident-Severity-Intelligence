...
def create_h3(df_in, h3_value):
    ...
    # H3_RES = 8
    df["h3"] = df.apply(
        lambda r: h3.latlng_to_cell(r["lat_norm"], r["lon_norm"], h3_value), axis=1
    )
...
def get_accidents_incidence(df):
    ...
    g = df.groupby(["h3", "tbin"], as_index=False).agg(accident_count=("id", "size"))
...
def ohe_cat_feats(df):
    ...
    feat = tmp_oh.groupby(["h3", "tbin"], as_index=False).mean(numeric_only=True)
...
def zero_inflation(df, data):
    ...
    grid = pd.MultiIndex.from_product([h3_cells, time_bins]).to_frame(index=False)
...
def h3_transformation():
    ...
    for name, df in grid_dict.items():
        ...
        logger.info("Getting accident's incidence (%s)", name)
        df_2 = get_accidents_incidence(df)

        # (5) Before_Sparsity_check on dfs
        logger.info("BEFORE INFLATION -- Sparsity check on %s", name)
        sparsity_check(df_2)
        ...
        logger.info("AFTER INFLATION -- Sparsity check on %s", name)
