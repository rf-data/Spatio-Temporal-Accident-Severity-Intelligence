...
def create_classification_report(y_true, y_pred):
    ...
    model_name = session.model_class     # gen_params.get("log_file", None)
    ...
    logger.info("Created Classification Report (model: %s)\n%s", 
                model_name,
                report)
    # print()
    # log ClassReport
...
def create_confusion_matrix(y_true, y_pred):
    ...
    logger.info(
        "Confusion Matrix created (model: %s):\ntn: %s | fn: %s | tp: %s | fp: %s",
        model_name,
...
def find_best_threshold(y_true, y_proba, metric, beta=2):
    # setup logger
    logger = session.logger
    model_name = session.model_class

    logger.info("Start 'threshold sweep (metric: %s)' on '%s' results",
                metric,
                model_name)    
    # 
    ...
    logger.info("Find best '%s' score (%s) @ t=%s",
                metric,
                scores[best_idx],
                thresholds[best_idx])
